...
| 🎚️ **انتخاب کیفیت پلی‌لیست** | Playlist quality | انتخاب کیفیت ویدئو (4K/1080p/…) یا **فرمت صوتی** (MP3/M4A/Opus/FLAC/WAV) |
...
| 🐢 **محدودیت سرعت** | Speed limit | تعیین حداکثر سرعت دانلود (KB/s) — مفید برای اینترنت محدود |
| 🔔 **اعلان سیستم** | System notifications | اعلان ویندوز هنگام پایان یا خطای دانلود (+ آیکون tray) |
| 💬 **خطاهای دوستانه** | Friendly errors | ترجمهٔ خطاهای yt-dlp به پیام فارسی قابل فهم |
| ⬆️ **بررسی به‌روزرسانی** | yt-dlp update check | بررسی نسخهٔ جدید yt-dlp از PyPI |
| ⚙️ **تب تنظیمات** | Settings tab | متمرکزسازی تنظیمات (ذخیره‌سازی، کوکی، پراکسی، سرعت، اعلان، ابزارها) |
| 🔍 **جستجو در پلی‌لیست** | Playlist search | فیلتر ویدئوهای پلی‌لیست بر اساس عنوان یا مدت |
| 📥 **دانلود دسته‌ای** | Batch download | چسباندن چند لینک (هر خط یکی) و افزودن همه به صف |
| 🖼️ **اطلاعات ویدئو** | Video info | تصویر بندانگشتی + کانال + مدت + بازدید + لایک |
| ▶️ **ادامهٔ دانلود** | Resume | ادامهٔ فایل‌های ناقص از همان‌جا (قابل خاموش‌کردن) |
...
<div align="center">
  ...
  <br>
  <img src="docs/settings_tab.png" alt="تب تنظیمات" width="80%" />
  <p><em>تب «⚙️ تنظیمات» — Settings tab</em></p>
</div>
...
4. **در تب «⚙️ تنظیمات»** مسیر ذخیره، کوکی، پراکسی، محدودیت سرعت، اعلان‌ها و بررسی به‌روزرسانی را تنظیم کنید.
...
3. **کیفیت دلخواه** را از کامبو انتخاب کنید — هم کیفیت‌های ویدئویی (4K/1440p/1080p/720p/…) و هم **فرمت‌های صوتی**:
   `🎵 بهترین صدا (بدون تبدیل)` ، `MP3 128/192/320 kbps` ، `M4A (AAC)` ، `Opus` ، `FLAC (بی‌اتلاف)` ، `WAV`
4. از کادر **«🔍 جستجو»** برای فیلتر کردن ویدئوها بر اساس عنوان یا مدت استفاده کنید.
5. ویدئو(های) موردنظر را **تیک** بزنید (یا «انتخاب همه»).
6. روی «دانلود انتخاب‌شده» بزنید — ویدئوی هر پلی‌لیست در پوشه‌ای به نام همان پلی‌لیست ذخیره می‌شود.

> 📏 حجم هر ویدئو بر اساس کیفیت انتخابی به‌صورت تدریجی محاسبه و نمایش داده می‌شود.
> ✅ ویدئوهای دانلودشده با **تیک سبز** مشخص می‌شوند.

### 📥 دانلود دسته‌ای (چند لینک)

در تب دانلود، بخش «۳) دانلود دسته‌ای» را ببینید:

1. چند لینک یوتیوب را، **هر کدام در یک خط**، در کادر بچسبانید.
2. روی **«➕ افزودن همه به صف»** بزنید — همهٔ لینک‌های معتبر به صف اضافه و پشت‌سرهم دانلود می‌شوند.
3. کیفیت پیش‌فرض برای این حالت «بهترین کیفیت» است (چون استخراج اولیه انجام نمی‌شود).

> 💡 این روش برای دانلود سریع یک لیست لینک که نمی‌خواهید فرمت هرکدام را جدا انتخاب کنید، مناسب است.

### 🖼️ اطلاعات ویدئو

به‌محض زدن «استخراج فرمت‌ها»، بخش «📺 اطلاعات ویدئو» تصویر بندانگشتی، عنوان، کانال، مدت و آمار **بازدید/لایک** را نشان می‌دهد.
